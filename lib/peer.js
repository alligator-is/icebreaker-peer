...
function defaultUrl(addr, params) {
  ...
  if (p.pathname == null && params.appKey!=null) p.pathname = '/' + params.appKey
  params.encoding = params.encoding||'base64'
  if (p.auth == null && params.keys.publicKey!=null) p.auth = utils.encode(params.keys.publicKey,params.encoding)
  else if (p.auth == null && params.keys.id!=null) p.auth = utils.encode(params.keys.id,params.encoding)
  return url.format(p)
}
...
var Peer = module.exports = function Peer(params) {
  ...
  source.connect = function (addr, _params, cb) {
    connector.connect(defaultUrl(addr,params), params, cb)
  }
